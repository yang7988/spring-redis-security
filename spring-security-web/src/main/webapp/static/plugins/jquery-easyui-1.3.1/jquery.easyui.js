...
jQuery.extend({
            ...
            if (matchs1 && matchs2 && (matchs1.length == matchs2.length)) {
                ...
                for (var i = 0; i < matchs1.length; i++) {
                    ...
                    switch (sign.charAt(0)) {
                        case 'y':
                            date.setFullYear(field);
                            break;
                        case 'M':
                            date.setMonth(field - 1);
                            break;
                        case 'd':
                            date.setDate(field);
                            break;
                        case 'H':
                            ...
                            break;
                        case 'm':
                            date.setMinutes(field);
                            break;
                        case 's':
                            date.setSeconds(field);
                            break;
                        case 'l':
                            date.setMilliseconds(field);
                            break;
                    }
                }
                return date;
    ...
    modifiers: {
        formatNumber: function (number, precision) {
            return (parseFloat(number) >= 0 ? parseFloat(number).toFixed(precision ? precision : 0) : '');
        },
...
(function ($) {
    ...
    ;
    function _dc(_dd, ul, _de, _df, invoked) {//add invoked 回调函数是否被调用
         ...
         */
        //add
        if (typeof invoked == "undefined" || invoked == true) {
            var _e4 = null;
            if (_dd != ul) {
                var _e5 = $(ul).prev();
                _e4 = _a6(_dd, _e5[0]);
    ...
    function _12e(_12f, _130) {
        ...
        _dc(_12f, ul[0], _130.data, true, _130.invoked);//add _130.invoked
        _d2(_12f, ul.prev());
    }
    ;
...
(function ($) {
    ...
    ;
    function _222(_223, _224, _225) {//add timeout
    ...
    $.messager = {
        ...
        alert: function (_228, msg, icon, fn, timeout) {//add timeout
            var _229 = "<div class=\"messager-text\">" + msg + "</div>";
            switch (icon) {
                case "error":
                ...
                case "warning":
                    ...
                    break;
                case "ok":
                    _229 = "<div class=\"messager-icon messager-ok\"></div>"
                        + _229;
                    break;
            }
            ...
            if ((icon == undefined || icon == "info") && timeout == undefined) {
                timeout = 1000;
            }
            var win = _222(_228, _229, _22a);//add timeout
            win.window({
                onOpen: function () {
                    var timer = null;
                    if (timeout > 0) {
...
(function ($) {
    ...
    function _2e9(_2ea) {
        ...
        //add
        var opts = _2eb.center.panel("options");
        _2eb.center.panel("resize", {
            top: cpos.top + opts.marginTop,
                ...
                .resizable({
                    ...
                    onStartResize: function (e) {
                        ...
                        if (dir == "north") {
                            ...
                            //add
                            pos.top = parseInt(_2fa.css("top")) + _2fa.height();
                            ...
                            if (dir == "south") {
                                ...
                                //add
                                pos.top = parseInt(_2fa.css("top")) - _2fc.height();
                                pos.left = parseInt(_2fa.css("left"));
                                pos.width = _2fa.outerWidth();
                                pos.height = _2fc.height();
                            } else {
                                if (dir == "east") {
                                    ...
                                    //add
                                    pos.left = _2fa.css("left") ? parseInt(_2fa.css("left")) - _2fc.width() : 0;
                                    ...
                                    if (dir == "west") {
                                        ...
                                        //add
                                        pos.left = (parseInt(_2fa.css("left")) || 0) + _2fa.width();
                                        pos.width = _2fc.width();
                                        pos.height = _2fa.outerHeight();
                                    }
                                }
    ...
    function _305(_306, _307, _308) {
        ...
        function _30b(dir) {
             ...
             */
            //add
            var opts = _309[dir].panel('options');
            var margin = {
            ...
            var p = $("<div></div>").appendTo(_306).panel($.extend({}, {
                cls: "layout-expand",
                title: "&nbsp;",
                closed: true,
                doSize: false,
        ...
        function _30d() {//收缩面板
            ...
            if (_307 == "east") {
                //add
                var opts = _309["east"].panel("options");
                var copts = _309["center"].panel("options");
                //end add
                return {
                    resizeC: {//计算east收缩后center的width
                        //width : _309.center.panel("options").width + _309["east"].panel("options").width - 28
                        width: copts.width + opts.width - 28
                    },
                    expand: {//计算east收缩后再浮动张开的left
                        //left : cc.width() - _309["east"].panel("options").width
                        left: cc.width() - opts.width - opts.marginRight
                    },
                    expandP: {//计算expandEast的size
                    ...
                    collapse: {//计算east向右移动目标终点的left，等同于expandP的left
                        //left : cc.width()
                        left: cc.width() - 28 - opts.marginRight
                    }
                };
            } else {
                if (_307 == "west") {//计算west收缩后center的width和left
                    //add
                    var opts = _309["west"].panel("options");
                    var copts = _309["center"].panel("options");
                    //end add
                    return {
                        resizeC: {
                            ...
                            width: copts.width + opts.width - 28,
                            //left : 28
                            left: 28 + opts.marginRight + opts.marginLeft + copts.marginLeft
                        },
                    ...
                    if (_307 == "north") {
                        //add
                        var opts = _309["north"].panel("options");
                        var sopts = _309["south"].panel("options");
                        //end add
                        //var hh = cc.height() - 28;
                        var hh = cc.height() - 28 - opts.marginTop - opts.marginBottom;
                        if (_2f0(_309.expandSouth)) {
                            //hh -= _309.expandSouth.panel("options").height;
                            hh -= _309.expandSouth.panel("options").height + sopts.marginTop + sopts.marginBottom;
                        } else {
                            if (_2f0(_309.south)) {
                        ...
                        //add
                        var eopts = _309["east"].panel("options");
                        _309.east.panel("resize", {
                            //top : 28,
                            top: 28 + opts.marginTop + opts.marginBottom + eopts.marginTop,
                            //height : hh
                            height: hh - eopts.marginTop - eopts.marginBottom
                        });
                        //add
                        var wopts = _309["west"].panel("options");
                        _309.west.panel("resize", {
                            //top : 28,
                            top: 28 + opts.marginTop + opts.marginBottom + wopts.marginTop,
                            //height : hh
                        ...
                        return {
                            resizeC: {
                                //top : 28,
                                top: 28 + opts.marginTop + opts.marginBottom + copts.marginTop,
                                //height : hh
                        ...
                        if (_307 == "south") {
                            //add
                            var opts = _309["south"].panel("options");
                            var nopts = _309["north"].panel("options");
                            //end add
                            //var hh = cc.height() - 28;
                            var hh = cc.height() - 28 - opts.marginTop - opts.marginBottom;
                            if (_2f0(_309.expandNorth)) {
                                //hh -= _309.expandNorth.panel("options").height;
                                hh -= _309.expandNorth.panel("options").height + nopts.marginTop + nopts.marginBottom;
                            } else {
                                if (_2f0(_309.north)) {
    ...
    function _30f(_310, _311) {//张开面板
        ...
        function _314() {
            ...
            if (_311 == "east" && _312.expandEast) {
                //add
                var opts = _312["east"].panel("options");
                return {
                    collapse: {
                        //left : cc.width()
                        left: cc.width() - 28 - opts.marginRight
                ...
                if (_311 == "west" && _312.expandWest) {
                    //add
                    var opts = _312["west"].panel("options");
                    return {
                        collapse: {
                    ...
                    if (_311 == "north" && _312.expandNorth) {
                        //add
                        var opts = _312["north"].panel("options");
                        return {
                            collapse: {
                        ...
                        if (_311 == "south" && _312.expandSouth) {
                            //add
                            var opts = _312["south"].panel("options");
                            return {
                                collapse: {
                                    //top : cc.height()
                                    top: cc.height() - 28 - opts.marginBottom
...
(function ($) {
    ...
    function _387(_388) {
        ...
        //固定电话或手机号码文本框，应实时弹出样式信息（保持原样）；
        if("phone"==_389.options.validType||"telephone"==_389.options.validType||"mobile"==_389.options.validType)
        {
        ...
        //身份证输入框，当离开焦点或点击【下一步】后再提示校验结果；
        else if("identity"==_389.options.validType)
        {
        	box.unbind(".validatebox").bind("focus.validatebox",function () {
        		_389.validating = false;
                _38a(_388);
...
//            _389.value = undefined;
//            if(_389.timer) {
//                clearTimeout(_389.timer);
//                _389.timer = undefined;
//            }
//            _389.timer = setTimeout(function() {
//                _390(_388);
//            }, _389.options.delay||1);
            ...
            length: {
                validator: function (_39e, _39f) {
                    ...
                    for (var i = 0; i < _39f.length; i++) {
                        this.message = this.message.replace(new RegExp("\\{" + i + "\\}", "g"), _39f[i]);
                    }
                    }
                    else
                    {
...
(function ($) {
    ...
    $.fn.numberbox.defaults = $.extend({}, $.fn.validatebox.defaults, {
        ...
        onBeforeFix: function (min, max, precision, val) {//add
            return true;
        },
        onAfterFix: function (min, max, precision, val) {//add
        }
    });
})(jQuery);
(function ($) {
...
(function ($) {
    ...
    ;
    function _4c2(_4c3, _4c4, _4c5, event) {
        ...
        opts.onSelect.call(_4c3, _4c4, data.rows[_4c4], event);
        if (tr.length) {
...
(function ($) {
    ...
    function _711(_712) {
        ...
        if (!opts.disabled) {
            _716.bind("mousedown.combo",function (e) {
            	//add
            	$(document).trigger("mousedown.qtip");
                e.stopPropagation();
            ...
            if (opts.editable) {
                ...
                _714.bind("click.combo",function () {
                    ...
                    if (_715.is(":visible")) {
                        ...
                        _717.addClass("combo-arrow-hover");
                    }).bind("mouseleave.combo",function () {
                        _717.removeClass("combo-arrow-hover");
                    }).bind("mousedown.combo", function () {
                    	//add
                    	$(document).trigger("mousedown.qtip");
                        return false;
    ...
    $.fn.combo = function (_744, _745) {
        if (typeof _744 == "string") {
            ...
            //add
            var method = $.fn.combo.methods[_744];
            if (method) {
                return method(this, _745);
            }
            else {
                return this.validatebox(_744, _745);
            }//end add
        }
        _744 = _744 || {};
        return this.each(function () {
...
(function ($) {
    ...
    function _752(_753) {
        ...
        if (item.length) {
            //当其获取焦点时，能使用键盘的上下键来选择数据。李越 2014-10-20
            ...
            item = _754.find("div.combobox-item:last");
//            item = _754.find("div.combobox-item:visible:last");
    ...
    function _758(_759) {
        ...
        if (item.length) {
            ...
            //当其获取焦点时，能使用键盘的上下键来选择数据。李越 2014-10-20
            item = _75a.find("div.combobox-item:first");
    ...
    function _76b(_76c, data, _76d) {
        ...
        //add
        if (opts.nullable) {
            $("<div class=\"combobox-item\" value=\"-1\"></div>").appendTo(_76e);
        }
        for (var i = 0; i < data.length; i++) {
            ...
            onLoadSuccess: function (node, data) {
                ...
                if (opts.nullable) {
                    if (node == null || typeof node == 'undefined') {
                        if (data.length > 0) {
                            var first = tree.tree("find", data[0].id);
                            tree.tree("insert", {
                                before: first.target,
                                data: {
                                    id: "-1",
...
(function ($) {
    function _7bb(_7bc) {
        ...
        function _7be() {
            ...
            _7bd.calendar.calendar({
                ...
                border: false,
                onSelect: function (date) {
                    //var _7c0 = opts.formatter(date, opts.format);
                    //_7c4(_7bc, _7c0);
                    _7c4(_7bc, $.date.format(date, "yyyy-MM-dd"));
            ...
            _7c1.find(".datebox-current").click(function () {
                ...
                //_7bd.calendar.calendar('options').onSelect.call(_7bc, date);
                //end add
            });
    ...
    function _7c5(_7c6) {
        ...
        //var _7c7 = opts.formatter(c.calendar("options").current, opts.format);
        //_7c4(_7c6, _7c7);
        _7c4(_7c6, $.date.format(c.calendar("options").current, "yyyy-MM-dd"));
    ...
    function _7c4(_7c8, _7c9) {
        ...
        //add
        var date = $.date.parse(_7c9, "yyyy-MM-dd");
        //$(_7c8).combo("setValue", _7c9).combo("setText", _7c9);
        $(_7c8).combo("setValue", _7c9).combo("setText", opts.formatter(date, opts.format));
        //_7ca.calendar.calendar("moveTo", opts.parser(_7c9));
        _7ca.calendar.calendar("moveTo", (date == null ? new Date() : date));
    }
    ;
    ...
    $.fn.datebox.parseOptions = function (_7d0) {
        ...
        return $.extend({}, $.fn.combo.parseOptions(_7d0), $.parser.parseOptions(_7d0, ["format"]));
    };
    $.fn.datebox.defaults = $.extend({}, $.fn.combo.defaults, {
        format: "yyyy-MM-dd", //add
        panelWidth: 180,
        panelHeight: "auto",
        keyHandler: {
            up: function () {
        ...
        closeText: "Close",
        okText: "Ok",
        formatter: function (date, format) {//add format
            //delete
            /*
        ...
        parser: function (s, format) {
             ...
             */
            //add
            var result = $.date.parse(s, format);
...
(function ($) {
    ...
    function _7dc(_7dd) {
        ...
        _7d7(_7dd, opts.formatter.call(_7dd, date, "yyyy-MM-dd HH:mm:ss"));
    ...
    function _7d7(_7de, _7df, _7e0) {
        ...
        if (!_7e0) {
            if (_7df) {
                ...
                var date = opts.parser.call(_7de, _7df, "yyyy-MM-dd HH:mm:ss");
                //$(_7de).combo("setValue", opts.formatter.call(_7de, date));
                $(_7de).combo("setValue", _7df);
                //$(_7de).combo("setText", opts.formatter.call(_7de, date));
                $(_7de).combo("setText", opts.formatter.call(_7de, date, opts.format));
            } else {
    ...
    $.fn.datetimebox.defaults = $.extend({}, $.fn.datebox.defaults,
        {
            format: "yyyy-MM-dd HH:mm:ss", //add
            showSeconds: true,
            timeSeparator: ":",
            keyHandler: {
                up: function () {
            ...
            formatter: function (date, format) {//add format
                /*
                 ...
                 return r;
                 */
                return $.fn.datebox.defaults.formatter(date, format);
            },
            parser: function (s, format) {//add format
                /*
                 if ($.trim(s) == "") {
                 return new Date();
                 }
                 ...
                 */
                //add
                return $.fn.datebox.defaults.parser(s, format);
            }
        });
})(jQuery);
(function ($) {
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        integer: {
            validator: function (value, params) {
                var reg = /^-?\d+$/;
                if (value) {
                    return $.fn.validatebox.defaults.rules.length.validator(value, params) && reg.test(value);
                }
                else {
                    return true;
                }
        ...
        percent: {
            ...
            message: "仅接受0~1之间的正浮点数，且长度必须在 {0} 和 {1} 之间。"
        },
        doublepz: {
            validator: function (value, params) {
                var reg = /^\d+(\.\d+)?$/;
        ...
        digit: {
            validator: function (value, params) {
                ...
                {
                    this.message="请输入有效的数字，且长度必须为{0}位";
                }
                else{
                    this.message="请输入有效的数字，且长度必须在 {0} 和 {1} 之间。";
                }
                var reg = /^\d+$/;
                if (value) {
                    return $.fn.validatebox.defaults.rules.length.validator(value, params) && reg.test(value);
        ...
        digitletter: {
            validator: function (value, params) {
                if(params[0]==params[1])
                {
                    this.message="仅接受数字和字母，且长度必须为{0}位";
                }
                var reg = /^[0-9A-Za-z]+$/;
                if (value) {
                    return $.fn.validatebox.defaults.rules.length.validator(value, params) && reg.test(value);
        ...
        phone: {
            validator: function (value, params) {
                var reg = /(^((\d{3,4}-)|(\(\d{3,4}\)))?\d{7,8}(-\d{1,4})?$)|(^0?1[34578]\d{9}$)/;
                if (value) {
                    return $.fn.validatebox.defaults.rules.length.validator(value, params) && reg.test(value);
                }
                else {
...
//            message: "移动电话格式错误。"
        ...
        text: {
            validator: function (value, params) {
                if(params[0]==params[1])
                {
                    this.message="请输入有效的文本，且长度必须为{0}位";
                }
                else{
                    this.message="请输入有效的文本，且长度必须在 {0} 和 {1} 之间。";
                }
        ...
        region: {
            ...
            message: "仅接受0~100之间(包括0和100)的正整数。"
        },
        /**
         * add by liyue
         * extends the text validator
         */
        newtext: {
            validator: function (value, params) {
                if(params[0]==params[1])
                ...
                var reg = /\w*((\%27)|(\'))\s*((((\%6F)|o|(\%4F))((\%72)|r|(\%52)))|union|select|insert|update|delete|drop)|((exec|execute)(\s|\+)+\w+)/i;
                if (value) {
                    return $.fn.validatebox.defaults.rules.newlength.validator(value, params) && !reg.test(value);
                }
                else {
                    return true;
                }
...
//                    return $.trim(value).length >= params[0] && $.trim(value).length <= params[1];
//                }
//                else {
//                    return true;
//                }
...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    newlength: {
        validator: function (value, params) {
            if (typeof value !=='undefined' && params && typeof params[0] !=='undefined' && typeof params[1] !=='undefined') {
                return $.trim(value).length >= params[0] && $.trim(value).length <= params[1];
            }
            else {
                return true;
    ...
    CHSLength: {
        validator: function (value, param) {
            ...
            this.message = '请输入有效的汉字，长度必须在 {0}和 {1}之间';
            if (param) {
                for (var i = 0; i < param.length; i++) {
                    this.message = this.message.replace(new RegExp("\\{" + i + "\\}", "g"), param[i]);
                }
        ...
        message: "请输入有效的汉字，长度必须在 {0}和 {1}之间"
    },
    ZIP: {
        validator: function (value, param) {
...
//    },
    ...
    //姓名验证(姓名只能输入汉字、英文字母（可包含一个或多个单词）、连接线(-)、点(·)和下划线(_))
    //2014-06-17改为
    //姓名验证(姓名只能输入汉字、英文字母（可包含一个或多个单词）、连接线(-)、中心点(·)和下划线(_))
    userName: {
        validator: function (value, param) {
        	//var regval = /(^([A-Za-z]+\s?)*[A-Za-z]$)|(^[\u0391-\uFFE5A-Za-z-·_]+$)/;
        	var regval = /(^[\u4e00-\u9fa5a-zA-Z_]+([·.]?[\u4e00-\u9fa5a-zA-Z_]+)+$)/;
        	return $.fn.validatebox.defaults.rules.length.validator(value, param) && regval.test(value);    
        },
        message: "请输入正确的姓名，长度必须在 {0}和 {1}之间"
    },
    dynamicID: {
        validator: function (value, params) {
            if (value && $.trim(value) != '' && params.length > 0) {
                var parentObj = $('#' + params[0]), idType = parentObj.combobox("getValue");
                if (idType && $.trim(idType)!='') {

                }
                //根据证件类型验证规则
                switch (idType) {
                    case "1":
                        this.message="“身份证件号码”格式错误";
                        return $.fn.validatebox.defaults.rules.identity.validator(value);//居民身份证
                        break;
                    case "2":return true;//军官证
                        break;
                    case "3":return true;//士兵证
                        break;
                    case "4":return true;//文职干部证
                        break;
                    case "5":return true;//部队离退休证
                        break;
                    case "6":return true;//香港特区护照/身份证明
                        break;
                    case "7":return true;//澳门特区护照/身份证明
                        break;
                    case "8":return true;//台湾居民来往大陆通行证
                        break;
                    case "9":return true;//境外永久居住证
                        break;
                    case "A":return true;//护照
                        break;
                    case "B":return true;//户口簿
                        break;
                    case "Z":return true;//其他
                        break;
                }
            }
        },
