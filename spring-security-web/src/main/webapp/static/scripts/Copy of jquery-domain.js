(function($) {
    ...
    function openEditDialog(target){
        ...
        if (!node) {
            top.$.messager.alert("信息", "很抱歉，尚未选择 " + opts.title + " 。", "info", null, 2000);
            return;
        }
 ...
 */
    function collect(target) {
        ...
        var collection = {};
        t.find(":input[name]").each(function(ndx, ele) {
            var box = $(this);
    ...
    function create(target, options) {
        ...
        $.ajax({
            ...
            success: function(data, statusText, XHR) {
            	var m = $.format("恭喜您，添加 <i>{0}</i> {1} 成功。", (opts.data.code || opts.data.name || ""), opts.title);
            	if(data!=1)
            	{
            		m = $.format("很遗憾，添加 <i>{0}</i> {1} 失败。", (opts.data.code || opts.data.name || ""), opts.title);
            	}
            	top.$.messager.show({
                    title: "信息", 
                    msg: m, 
 ...
 */
    function edit(target, options) {
        ...
        $.ajax({
            ...
            success: function(data, statusText, XHR) {
                var m = $.format("恭喜您，编辑 <i>{0}</i> {1} 成功。", (opts.data.code || opts.data.name || ""), opts.title);
                top.$.messager.show({
                    title: "信息", 
                    msg: m, 
                    timeout: 4000, 
        ...
        top.$.messager.confirm("确认", m, function(result) {
            if (result) {
                $.ajax({
                    ...
                    success: function(data, status, XHR) {
                        m = $.format("恭喜您，删除 <i>{0}</i> {1}成功。", text, title);
                        top.$.messager.show({
                            title: "信息", 
                            msg: m, 
                            timeout: 4000, 
    ...
    function datagrid(target, options) {
        ...
        $(target).datagrid($.extend({}, opts, {
            ...
            onLoadSuccess: function(data, status, XHR) {
                ...
                //                    top.$.messager.show({ title: "信息", msg: "很抱歉，没有 " + (opts.title || "") + " 记录。", timeout: 4000, showType: "slide" });
                }
                    ...
                    $(".datagrid-view2 .datagrid-row-selected").qtip({
                        ...
                        events : {
                            show : function(e,api){
                                ...
                                if (mouseX + $(".qtip").width() < $("body").width()){
                                    $(".qtip").css("margin-left", -(($("body").width() - $(".qtip").width()) / 2 - mouseX) + 5);
                                }else{
                                    $(".qtip").css("margin-left", ($("body").width() - $(".qtip").width()) / 2 - 22);
                                }
 ...
 */
    ...
    $.fn.domain.methods = {
        ...
        openCreateDialog: function(jq) {
            return jq.each(function() {
                openCreateDialog(this);
            });
        },
        openEditDialog: function(jq) {
            return jq.each(function() {
                openEditDialog(this);
            });
        },
